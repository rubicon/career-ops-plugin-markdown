...
export function parseCvMarkdown(markdown) {
  // Drop the document's own final newline before splitting. Everywhere else the
  // empty trailing element it produces is skipped as a blank line, but inside a
  // fence still open at EOF it was kept as content, so every unclosed block gained
  // a blank last line nobody wrote. A blank line the author did write survives.
  ...
  // The innermost container currently open. Content goes here so that it keeps the
  // position the author gave it: holding a code block at section level while
  // bullets went on attaching to the open entry reordered the document, because
  // the entry renders in full before the section reaches the block.
  const openContent = () => (subrole ? subrole.content : entry ? entry.content : section?.blocks);

  // A fenced block is content, never structure. It needs a container, so one
  // written above the first section (where the only container is the single-line
  // contact block) is dropped rather than mangled into it. An empty block is still
  // something the author wrote, so a container that exists keeps it.
  const closeFence = () => {
    openContent()?.push({ type: 'code', info: fenced.info, lines: fenced.lines });
    fenced = null;
  };
  ...
  for (const raw of lines) {
    ...
    if (fenced) {
      ...
      if (close && close[1][0] === fenced.fence[0] && close[1].length >= fenced.fence.length) {
        ...
        // Kept verbatim in the sense that matters here -- no line inside a block
        // is read as a heading, a bullet or a contact field -- but not byte for
        // byte: a trailing-whitespace-free document is part of the output
        // contract above, alongside normalized dashes and rewritten fences.
        // Blocks.push strips per line at render time regardless, so dropping this
        // would not preserve the whitespace, only move where it disappears.
    ...
    if (h4) {
      ...
      subrole = { type: 'subrole', title, role: null, date: null, content: [] };
    ...
    if (bullet) {
      openContent().push({ type: 'bullet', text: keepInlineMarkdown(bullet[1]) });
      continue;
    }
    ...
    // Loose prose inside a role reads as one more of its bullets; at section
    // level it stays a paragraph.
    const text = keepInlineMarkdown(line);
    openContent().push(subrole || entry ? { type: 'bullet', text } : { type: 'paragraph', text });
  }
...
/**
 * Render one ordered run of content in the order it was written. Consecutive
 * bullets group into a single list, and anything else -- a paragraph, a code
 * block, a role entry -- flushes that list first, so a block written between two
 * bullets renders between them instead of after both. The same walk serves a
 * section, a role entry and a sub-role, which is what keeps their content in
 * position at every depth.
 * @param {Blocks} doc
 * @param {Array} blocks
 */
function renderContent(doc, blocks) {
  const pending = [];
  const flush = () => {
    if (pending.length) doc.push(bulletList(pending.splice(0)));
  };
...
function renderMarkdown(cv) {
  ...
  for (const section of cv.sections) {
    ...
    renderContent(doc, section.blocks);
  }
