...
/**
 * Match a fence opener/closer, capturing the run of backticks or tildes and any
 * info string. Up to three leading spaces still open a fence (CommonMark).
 */
...
export function parseCvMarkdown(markdown) {
  ...
  // the single-line contact block) is dropped rather than mangled into it.
  const closeFence = () => {
    if (section && fenced.lines.length) {
  ...
  for (const raw of lines) {
    ...
    if (open) {
      fenced = { fence: open[1], info: open[2].trim(), lines: [] };
      continue;
    }
